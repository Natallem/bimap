...
struct node_bimap : Node<Left, left_tag>, Node<Right, right_tag> {
  ...
  node_bimap(const Left &left, const Right &right)
      : Node<Left, left_tag>(std::forward<Left>(left)), Node<Right, right_tag>(
                                                            std::forward<Right>(
                                                                right)){};
...
public:
  ...
  struct left_iterator {
    ...
    left_iterator(const bimap *itr_bimap, node_t *node) noexcept
        : itr_bimap(itr_bimap), itr_node(node){};

    left_iterator(const bimap *itr_bimap, left_node_t *node) noexcept
        : itr_bimap(itr_bimap), itr_node(static_cast<node_t *>(node)){};
  ...
  private:
    ...
    friend T1
    bimap<Left, Right, CompareLeft, CompareRight>::common_erase(T1 &, T2 &,
                                                                T3 &) noexcept;
  };
  ...
  bimap(bimap const &other)
      : left_tree(other.left_tree), right_tree(other.right_tree),
  ...
  left_iterator insert(left_t const &left, right_t const &right) {
    return common_insert(std::forward<const left_t>(left),
...
private:
  ...
  iterator_t common_erase(iterator_t &it, tree_t &cur_tree,
                          other_tree_t &other_tree) noexcept {
  ...
  bool common_erase_bool(const val_t &value, tree_t &tree,
                         other_tree_t &other_tree) noexcept {
  ...
  template <typename iterator_t, typename tree_t, typename other_tree_t>
  iterator_t common_erase_interval(iterator_t &first, iterator_t &last,
                                   tree_t &cur_tree,
                                   other_tree_t &other_tree) noexcept {
  ...
  template <typename T, typename U> T *flip_nodes_types(U *ptr) const {
